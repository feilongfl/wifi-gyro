...
function setStr(data, offset, val)
    local t = {}
    for i = 1, lastData:len() do table.insert(t, lastData:byte(i)) end
...
function newArray(size)
    ...
    setmetatable(t,
                 {
        __index = function(_, i) return i >= 1 and i <= size and i or nil end
    })
    return t
end
...
function sendReq(reqdata, s, port, ip)
    ...
    -- timeDebug("7")
    if ip ~= nil then s:send(port, ip, str) end
    -- timeDebug("8")
end
